...
def findFields(dados):
    ## Finds all fields in first level sub arrays
    fieldslist=[]
    for key1, value1 in dados.items():
        for key2, value2 in value1.items():
            if not key2 in fieldslist:
                fieldslist.append(key2)
    return fieldslist

def regswithoutfield(regs, fieldname):
    count=0
    for reg in regs:
        if not reg.has_key(fieldname):
            count+=1
    return count
